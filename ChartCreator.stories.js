...
const generateData = () => {
  ...
  for (let i = 0; i < 55; i++) {
    dataGenerated.push({
      ...
      avgRelBidAskSpread: parseFloat(
        (Math.random() * (5 - 1 + 1) + 1).toFixed(2)
      ),
      adtvQty: parseFloat((Math.random() * (5 - 1 + 1) + 1).toFixed(2)),
      adtvAmnt: parseFloat((Math.random() * (5 - 1 + 1) + 1).toFixed(2))
    });
  }
...
export default {
  ...
  component: ChartCreatorStory
};

const Template = () => <ChartCreatorStory />;
